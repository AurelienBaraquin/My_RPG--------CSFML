...
void draw_map_mini(global_t *global, int size)
{
    ...
    for (; i < global->map->size && i <= player->pos.y / 32 + size; i++) {
        ...
        for (; j < global->map->size && j <= player->pos.x / 32 + size; j++) {
            draw_map_tyle_mini(global, j, i);
        }
    }
}

void draw_mini_village_sprite(char c, global_t *global, int pos_x, int pos_y)
{
    if (c >= '1' && c != 't' && (c < 'A' || c > 'N')) {
        sfSprite_setPosition(global->sprites[get_village_type(c)]->sprite,
        (sfVector2f){pos_x * 32, pos_y * 32});
        sfRenderTexture_drawSprite(global->map->texture, global->sprites
...
void draw_mini_village(global_t *global, int size)
{
    ...
    for (; i < global->map->size && i <= player->pos.y / 32 + size; i++) {
        ...
        if (j < 0)
            j = 0;
        for (; j < global->map->size && j <= player->pos.x / 32 + size; j++) {
            draw_mini_village_sprite(global->map->village[i][j], global, j, i);
        }
    }
}
